...
#   $env:DICTATE_SKIP_WIZARD = "yes"         Skip interactive config wizard
#
...
function Write-DefaultConfig {
  ...
  if (-not (Test-Path $ConfigFile)) {
    @(
      ...
      '# Generated by install.ps1 — finish setup with `dictate config wizard`',
...
if ($env:DICTATE_SKIP_WIZARD -eq "yes") {
  ...
  try {
    & $BinPath config wizard
    Info "Configuration complete"
  } catch {
    Warn "Config wizard did not complete ($($_.Exception.Message))"
...
Code "dictate hotkeys --warm       # background agent, instant first press"
Code "dictate autostart install    # start agent on login"
Write-Host ""
