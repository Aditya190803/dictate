...
#   DICTATE_SKIP_WIZARD=yes     Skip interactive setup for automation
...
Environment variables:
  ...
  DICTATE_SKIP_WIZARD        Skip interactive setup (default: no)
EOF
  exit 0
}
...
# Generated by install.sh — finish setup with `dictate setup`
...
OUTPUT_MODE=$(get_config_value SHORTCUT_OUTPUT type)
PROFILE=$(get_config_value DICTATE_PROFILE segmented)
...
if "$BIN_PATH" shortcuts "$DESKTOP" --profile "$PROFILE" --mode "$OUTPUT_MODE" --key "$SHORTCUT_KEY" 2>/dev/null; then
  true
else
  warn "Could not generate shortcut snippet for desktop '${DESKTOP}'."
  code "Bind your key to: dictate"
fi

printf "\n${BOLD}Commands:${NC}\n"
code "dictate                 # start or stop"
code "dictate setup"
code "dictate doctor"
code "dictate config set KEY value"
code "dictate words"
